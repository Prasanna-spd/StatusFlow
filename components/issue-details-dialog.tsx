...
export default function IssueDetailsDialog({ isOpen, onClose, issue, onDelete = () => {}, onUpdate = () => {}, borderCol = "" }) {
      ...
      <DialogContent>
        <DialogHeader>
          <div className="flex justify-between items-center">
            ...
            {isProjectPage && (
              <Button variant="ghost" size="icon" onClick={handleGoToProject} title="Go to Project">
                <ExternalLink className="h-4 w-4" />
              </Button>
            )}
          </div>
        </DialogHeader>
        {(updateLoading || deleteLoading) && <BarLoader width={"100%"} color="#36d7b7" />}
        <div className="space-y-4">
          ...
          <div>
            ...
            <MDEditor.Markdown className="rounded px-2 py-1" source={issue.description ? issue.description : "--"} />
          </div>
          ...
          {(deleteError || updateError) && <p className="text-red-500">{deleteError?.message || updateError?.message}</p>}
        </div>
      </DialogContent>
    </Dialog>
  );
